...
# Embedded file name: Common\arxmlparse\artop_R19\entity\DdsFieldDeployment.py
from .ServiceFieldDeployment import ServiceFieldDeployment

class DdsFieldDeployment(ServiceFieldDeployment):

    def __init__(self):
        super().__init__()
        from .DdsMethodDeployment import DdsMethodDeployment
        from .DdsEventDeployment import DdsEventDeployment
        self._artop_get = None
        self._artop_notifier = None
        self._artop_set = None
        if not hasattr(self, "_tag_to_attr"):
            self._tag_to_attr = {}
        self._tag_to_attr.update({'_artop_get':"DDS-METHOD-DEPLOYMENT", 
         '_artop_notifier':"DDS-EVENT-DEPLOYMENT", 
         '_artop_set':"DDS-METHOD-DEPLOYMENT"})
