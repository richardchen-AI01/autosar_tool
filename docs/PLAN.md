...
reference/autosar-cfg/docs/04-recipes/15-add-new-param-end-to-end.md   ← 端到端补丁实战经验
```
...
主 sprint 之外、单独某个模块或单独某条线的深度复刻，不在本文件展开，单独成文档以便聚焦：

- [MEMIF_REPLICA_PLAN.md](MEMIF_REPLICA_PLAN.md) — MemIf 单模块的端到端复刻（含 IDE GUI、编辑、保存、auto-fill、round-trip 兼容性比对、反编填补策略）

新增子计划放在 `docs/` 下，命名 `<TOPIC>_PLAN.md`，并在这一节加一行链接。**不要再起并列的版本号 PLAN（如 PLAN_v0.x.md）**——v0.x 路线统一在 §9。
