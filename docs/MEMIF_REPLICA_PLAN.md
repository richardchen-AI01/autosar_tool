...
- 反编译参考项目里**和 MemIf 直接相关**的 .class，复刻不可见的语义
...
| 7 | `MemIfBswmdData.class` | schema 元数据加载 | ❌ 缺 | 反编 → 先尝试让 Sphinx 自动从 MemIfDef.arxml 加载（更倾向）；不行则复刻 | 中 |
| 8 | `MemIfMetaModelDescriptor.class` | Sphinx EMF 描述符 | ❌ 缺 | 反编 → 复刻一个最小实现 | 中 |
| 9 | `MemIfDefaultRegistry.class` | **表单布局** | ❌ 缺 | 反编 → 看是手写还是数据驱动；如数据驱动可让 Sphinx auto-render，否则需复刻 | **高** |
| 10 | `MemIfUpdateBswmd.class` | 旧 ARXML 升级到新 schema | ❌ 缺 | v0.1 不做；写 stub return null；记 known issue | 低（已知差距）|
| 11 | `MemIfFunctionExtension.class` | 业务 hook 入口注册 | ❌ 缺 | 反编 → 复刻（5-10 行注册逻辑）| 低 |
| 12 | `MemIfGeneralEaMapSupport.class` | **auto-fill NumberOfDevices**（EA 侧）| ❌ 缺 | 反编 → 复刻成 Java；保留我已加的 Python `derivedNumberOfDevices` 作为后端兜底 | **高** |
...
→ **必须反编的 4 项**：#9 / #11 / #12 / #13。

## 3. 反编译填补策略

### 工具

- **CFR**（推荐）—— 输出干净的 Java 源；命令：`cfr <jar>!<class> --outputdir decomp/`
- **Procyon**（备用）—— 处理 lambda / 内部类更稳

### 流程

1. 在 `tools/decompile-memif.sh` 里把上述 4 个 .class 一次性 decompile 到
   `docs/reference/decompiled-memif/`（gitignored）
2. **每个反编出来的源都做合法性 review**——标注 iSoft 私有 vs AUTOSAR-标准实现，
   写在 `docs/decisions/0006-decompile-scope.md`
3. **复刻为我们自己的代码**——逐句 paraphrase（变量名、注释、结构都改）；
   不直接 paste
4. 反编原文留在 `docs/reference/decompiled-memif/`（gitignored，本机参考），
   **复刻产物**放 `ide/modules/cn.com.myorg.bswbuilder.modules.memif/src/`
5. 验证手段：跑同一个 demo，比 ORIENTAIS 和我们的输出 / 行为是否一致

### 合法性边界

作为 V25.10 合法用户，本机反编做兼容研究是允许的；最终 ship 的是 paraphrase 后的代码，
**没有任何 .class 二进制或反编原文进入 git**。商业化路线必须等 v0.3 clean-room
（见 PLAN.md §9.3），届时连反编参考都不能再看。
...
### Phase C — Auto-fill（function extensions 反编 + 复刻）
...
| 项 | 内容 |
|---|---|
| 进入条件 | Phase B 通过 + 4 个 .class 已反编 + 复刻 + 合法性 review 通过 |
| 关键产物 | `MemIfGeneralEaMapSupport` / `FeeMapSupportEnable` Java 等价物；form listener 接 NvM 模型变更；与 Python `derivedNumberOfDevices` 行为一致 |
| 退出条件（自动） | 在 demo workspace 里加一个 NvMFeeRef block → 跑 script → 读 MemIf 表单的 NumberOfDevices 值 → 等于 ORIENTAIS 在同操作下输出的值 |
| 退出条件（手测） | 同上，肉眼看；并测 NvMEaRef 单独 / 两个都加 / 删除等 4 种组合 |
