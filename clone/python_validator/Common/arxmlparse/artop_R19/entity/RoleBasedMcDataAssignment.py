# uncompyle6 version 3.9.3
# Python bytecode version base 3.8.0 (3413)
# Decompiled from: Python 3.11.15 (main, Mar 11 2026, 17:14:47) [Clang 20.1.8 ]
# Embedded file name: Common\arxmlparse\artop_R19\entity\RoleBasedMcDataAssignment.py
from .ARObject import ARObject

class RoleBasedMcDataAssignment(ARObject):

    def __init__(self):
        super().__init__()
        from .RptExecutionContext import RptExecutionContext
        from .McDataInstance import McDataInstance
        from .VariationPoint import VariationPoint
        self._artop_role = None
        self._artop_executionContextRef = []
        self._artop_mcDataInstanceRef = []
        self._artop_variationPoint = None
        if not hasattr(self, "_tag_to_attr"):
            self._tag_to_attr = {}
        self._tag_to_attr.update({'_artop_executionContextRef':"RPT-EXECUTION-CONTEXT", 
         '_artop_mcDataInstanceRef':"MC-DATA-INSTANCE", 
         '_artop_variationPoint':"VARIATION-POINT"})

    @property
    def role_(self):
        return self._artop_role

    @property
    def ref_executionContexts_(self):
        return self._artop_executionContextRef

    @property
    def executionContexts_(self):
        return self._artop_executionContextRef

    @property
    def ref_mcDataInstances_(self):
        return self._artop_mcDataInstanceRef

    @property
    def mcDataInstances_(self):
        return self._artop_mcDataInstanceRef

    @property
    def ref_variationPoint_(self):
        return self._artop_variationPoint

    @property
    def variationPoint_(self):
        if self._artop_variationPoint is not None:
            if hasattr(self._artop_variationPoint, "uuid"):
                return self._artop_variationPoint.uuid
        return
