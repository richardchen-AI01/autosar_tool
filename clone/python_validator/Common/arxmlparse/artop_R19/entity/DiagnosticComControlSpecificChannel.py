...
# Embedded file name: Common\arxmlparse\artop_R19\entity\DiagnosticComControlSpecificChannel.py
...
class DiagnosticComControlSpecificChannel(ARObject):
    ...
    def __init__(self):
        ...
        self._artop_subnetNumber = None
        self._artop_diagnosticComControlClass = None
        self._artop_specificChannelRef = None
        if not hasattr(self, "_tag_to_attr"):
            self._tag_to_attr = {}
        self._tag_to_attr.update({'_artop_diagnosticComControlClass':"DIAGNOSTIC-COM-CONTROL-CLASS", 
         '_artop_specificChannelRef':"COMMUNICATION-CLUSTER"})

    @property
    def subnetNumber_(self):
        return self._artop_subnetNumber
    ...
    @property
    def ref_specificChannel_(self):
        return self._artop_specificChannelRef

    @property
    def specificChannel_(self):
        if self._artop_specificChannelRef is not None:
            if hasattr(self._artop_specificChannelRef, "uuid"):
                return self._artop_specificChannelRef.uuid
        return
