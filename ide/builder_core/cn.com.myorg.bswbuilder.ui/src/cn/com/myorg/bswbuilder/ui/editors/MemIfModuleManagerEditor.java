...
public class MemIfModuleManagerEditor extends FormEditor {
    ...
    public void init(IEditorSite site, IEditorInput input) throws PartInitException {
        ...
        if (isBswmdDescriptor(f)) {
            // BSWMD descriptor — schema/metadata, not a user-edited config.
            // Refuse to open in the module-config editor (matches reference V25.10
            // behavior where bswmds/ contents are filtered out of the navigator
            // entirely; this branch defends against direct opens via Show In / Open
            // With / search results).
            throw new PartInitException(
                    "Cannot edit BSWMD descriptor in MemIf Module Manager: "
                  + f.getName() + ".\n\n"
                  + "Files in bswmds/ describe module schemas and are read-only "
                  + "metadata (regenerated by 'Update Bswmd'). Open with the XML "
                  + "editor if you need to inspect them.");
        }
        this.arxmlFile = f;
    ...
    /**
     * True iff the file lives under a {@code bswmds/} directory or its name
     * ends in {@code _Bswmd.arxml}. Either heuristic catches reference
     * V25.10's BSWMD descriptor files.
     */
    private static boolean isBswmdDescriptor(File f) {
        if (f == null) return false;
        String name = f.getName();
        if (name.endsWith("_Bswmd.arxml")) return true;
        File parent = f.getParentFile();
        while (parent != null) {
            if ("bswmds".equals(parent.getName())) return true;
